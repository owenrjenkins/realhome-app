// RealHome API
// Loads CSV -> static filters -> crow-fly prefilter -> Distance Matrix verify -> return matched items
...
// Conservative speeds for radius prefilter (km/h)
const SPEED_KMH = { driving: 50, transit: 30, walking: 5, bicycling: 15 };

// Distance Matrix request batching
const DM_BATCH = 25;

// Cache origin|dest|mode -> seconds
const dmCache = new LRU({ max: 50_000, ttl: 1000 * 60 * 60 });

function toNum(n) { const v = Number(n); return Number.isFinite(v) ? v : NaN; }
  ...
  if (CSV_PATH && fs.existsSync(CSV_PATH)) {
    ...
    PROPS = rows
      .map((r, i) => {
        ...
        const lng = toNum(r.lng ?? r.lon ?? r.longitude ?? r.Longitude);
        if (!Number.isFinite(price) || !Number.isFinite(bedrooms)) return null;
        if (!isValidLatLng(lat, lng)) return null;
        return { id, address, postcode, price, bedrooms, lat, lng };
      })
      .filter(Boolean);
    console.log(`[INIT] Loaded ${PROPS.length.toLocaleString()} properties`);
  } else {
    console.warn("[WARN] PROPERTIES_CSV_PATH not set or file missing");
  }
} catch (e) {
  console.error("[ERROR] CSV load failed:", e);
  PROPS = [];
}
...
async function getDurationsSeconds(origin, dests, mode) {
  ...
  for (let i = 0; i < dests.length; i += DM_BATCH) {
    ...
    slice.forEach((d, idx) => {
      ...
      if (cached != null) out[i + idx] = cached;
      else { needIdx.push(idx); needDests.push(d); }
    });

    if (!needDests.length) continue;
...
app.post("/api/properties/travel-filter", async (req, res) => {
  try {
    ...
    const m =
      mode === "walking" || mode === "bicycling" || mode === "transit" || mode === "driving"
        ? mode
        : "driving";
    ...
    // 2) radius pre-filter
    const speed = SPEED_KMH[m] || SPEED_KMH.driving;
    const radiusKm = Math.max(1, (speed * maxSecs) / 3600);
    const pre = base.filter((p) => haversineKm(origin, { lat: p.lat, lng: p.lng }) <= radiusKm);

    // 3) cap by nearest crow-fly distance (tunable)
    const MAX_CANDIDATES_DEFAULT = 2000;
    const MAX_CANDIDATES_HARD_CAP = 10000;
    const requestedMax = Number(maxCandidates ?? req.query.maxCandidates ?? MAX_CANDIDATES_DEFAULT);
    const MAX_CANDIDATES = Math.min(Math.max(200, requestedMax || MAX_CANDIDATES_DEFAULT), MAX_CANDIDATES_HARD_CAP);
    ...
    // 4) verify travel time
    const dests = candidates.map((c) => ({ lat: c.lat, lng: c.lng }));
    const secs = await getDurationsSeconds({ lat: Number(origin.lat), lng: Number(origin.lng) }, dests, m);

    const passed = [];
    for (let i = 0; i < candidates.length; i++) {
      const c = candidates[i];
      const dur = secs[i];
      if (dur != null && dur <= maxSecs) passed.push({ ...c, travelSeconds: dur });
    }
